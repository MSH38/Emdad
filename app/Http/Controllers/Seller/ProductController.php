...
class ProductController extends Controller
{
    ...
    public function status_branch_update(Request $request)
    {
        if ($request['status'] == 0) {
            Product::where(['id' => $request['id'], 'added_by' => 'seller', 'user_id' => \auth('seller')->id()])->update([
                'status_branch' => $request['status'],
            ]);
            return response()->json([
                'success' => 1,
            ], 200);
    ...
    public function store(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $product->meta_title = $request->name[array_search('en', $request->lang)];
            $product->meta_description = $request->name[array_search('en', $request->lang)];
            $product->meta_image = ImageManager::upload('product/thumbnail/', 'png', $request->file('image'));
    ...
    function list(Request $request)
    {
        ...
        $sellerType = Seller::where(['id' => \auth('seller')->id()])->first();
        Log::info($sellerType);
        if($sellerType->id == auth('seller')->id() && $sellerType->seller_parent == 0){
            if ($request->has('search')) {
                $key = explode(' ', $request['search']);
                $products = Product::where(['added_by' => 'seller', 'user_id' => \auth('seller')->id(), 'child_seller_id' => 0])
            ...
            if ($request->has('search')) {
            $key = explode(' ', $request['search']);
            $products = Product::where(['added_by' => 'seller', 'child_seller_id' => \auth('seller')->id()])
                ->where(function ($q) use ($key) {
                    foreach ($key as $value) {
                        $q->Where('name', 'like', "%{$value}%");
                    }
    ...
    function list_haad_office(Request $request)
    {
        $sellerId = auth('seller')->id();
        $parentsellerId = seller::where('id', $sellerId)->first();
        $sellerIdFinal = $parentsellerId->seller_parent;

        $query_param = [];
        $search = $request['search'];
        if ($request->has('search')) {
            $key = explode(' ', $request['search']);
            $products = Product::where(['added_by' => 'seller', 'user_id' => $sellerIdFinal])
                ->where(function ($q) use ($key) {
                    foreach ($key as $value) {
                        $q->Where('name', 'like', "%{$value}%");
                    }
        ...
        return view('seller-views.product.listheadofice', compact('products', 'search'));
    }
     ...
     public function copy_product()
    {
        $cat = Category::where(['parent_id' => 0])->get();
        $br = Brand::orderBY('name', 'ASC')->get();
        return view('seller-views.product.copy-products', compact('cat', 'br'));
    }
    ...
    public function copy($id)
    {
        ...
        $categories = Category::where(['parent_id' => 0])->get();
        $br = Brand::orderBY('name', 'ASC')->get();
        return view('seller-views.product.copy-products', compact('categories', 'br', 'product', 'product_category'));

    }

    public function store_copy(Request $request, $id)
    {
        $copy_product = Product::withoutGlobalScopes()->with('translations')->find($id);
        $newProduct = $copy_product->replicate();
        $newProduct-> child_seller_id = auth('seller')->id();
        $newProduct->save();
    ...
    public function view($id)
    {
        ...
        $start = Date('Y-m-d', strtotime('-7 months'));
        $end = Date('Y-m-d');

        $begin = new DateTime($start);
        $endbegin = new DateTime($end);

        $mounth_count = [];
        for ($i = $begin; $i <= $endbegin; $i->modify('+1 month')){
            $mounth_qty = DB::table('order_details')->where([
                'seller_id' =>auth('seller')->id(),
                'product_id' => $id
            ])->where(DB::raw('DATE_FORMAT(created_at,"%M")'),$i->format('M'))
            ->sum('qty');

            array_push($mounth_count, ['month' => $i->format('M'),'value' => $mounth_qty]);

        }

        $product_prices = Product::where('added_by','seller')->get();

        $product_count = $product_prices->count('unit_price');
        $product_sum = $product_prices->sum('unit_price');
        $product_max = $product_prices->max('unit_price');
        $product_min = $product_prices->min('unit_price');
        $product_avg = $product_sum / $product_count;


        return view('seller-views.product.view', compact('product', 'reviews','mounth_count','product_avg','product_max','product_min'));
    }
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        Storage::disk('public')->put('products_file/' . $file_name, $request->file('products_file'));
    ...
    public function search(Request $request)
    {
        ...
        return view('seller-views.product.search.table_search',compact('products'));
