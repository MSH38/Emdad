...
class DashboardController extends Controller
{
    public function dashboard()
    {
        $top_sell = OrderDetail::with(['product'])
                                ...
                                ->select('product_id', DB::raw('SUM(qty) as count'),'qty')
                                ->groupBy('product_id')
                                ->orderBy("count", 'desc')
                                ->take(6)
                                ->get();

            // dd($top_sell[0]->product->name);
            // if(!isset($top_sell))
            // $top_sell = [] ;

        $last_orders = OrderDetail::with(['product','order'])->where('seller_id',auth('seller')->id())
        ->latest()
        ->limit(6)
        ->get();
        ...
        $data['today_order'] = DB::table('orders')->where('seller_id',auth('seller')->user()->id)->whereDate('created_at' , now()->format("Y-m-d"))->count();

        $data['last_orders'] = $last_orders;

        $todayAmount = DB::table('seller_wallet_histories')
                ->where('seller_id',auth('seller')->id())
                ->whereDate('created_at',now()->format('Y-m-d'))
                ->sum('amount');

        $customers = Order::where('seller_id',auth('seller')->id())->distinct()->get(['customer_id']);
        ...
        $days_amount = [];
        $days = [];
        for ($i = $begin; $i <= $endbegin; $i->modify('+1 day')){
            $day_amount = DB::table('order_transactions')->where([
                'seller_is'=>'seller',
                'seller_id'=>auth('seller')->id(),
                'status'=>'disburse'
            ])->where(DB::raw('DATE_FORMAT(created_at,"%d")'),$i->format('d'))
            ->sum('seller_amount');

            array_push($days, ['day' => $i->format('d M'),'value' => $day_amount]);
        }




        return view('seller-views.system.dashboard', compact('data','days','days_amount' ,'seller_data', 'commission_data','todayAmount','customers'));
    }
