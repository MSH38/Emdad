...
@section('content')
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{\App\CPU\translate('new req ordert')}}</li>
            </ol>
        </nav>

        @foreach($newReqOrder as $ticket )
            <?php
            $userDetails = \App\User::where('id', $ticket['customer_id'])->first();
            $conversations = \App\Model\NewReqOrderConv::where('support_ticket_id', $ticket['id'])->get();
            $admin = \App\Model\Admin::get();
            ?>
            <div class="media pb-4">
                <img class="rounded-circle" style="width: 40px; height:40px;"
                     src="{{asset('storage/public/profile')}}/{{isset($userDetails)?$userDetails['image']:''}}"
                ...
                <div class="media-body {{Session::get('direction') === "rtl" ? 'pr-3' : 'pl-3'}}">
                    ...
                    <p class="font-size-md mb-1">{{$ticket['subject']}}</p>
                                  <p class="font-size-md mb-1">{{$ticket['description']}}</p>
                                  <p class="font-size-md mb-1">{{$ticket['quantity']}}</p>
                                  <p class="font-size-md mb-1">{{$ticket['price']}}</p>
            </div>
            </div>
    ...
    <!-- Leave message-->
        ...
        @foreach($newReqOrder as $reply)
            <form class="needs-validation" href="{{route('admin.new-req-order.replay',$reply['id'])}}" method="post"
                  >
                @csrf
